...
export function UserMenu({ user }: { user: AuthUser }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-brand-accent to-indigo-600 text-xs font-semibold text-white ring-2 ring-brand-bg">
          {initials(user)}
        </span>
      </button>
        ...
        {open && (
          ...
          >
            <div className="flex items-center gap-3 border-b border-brand-border/50 px-2.5 pb-2.5 pt-1.5">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-gradient-to-br from-brand-accent to-indigo-600 text-xs font-semibold text-white">
                {initials(user)}
              </span>
